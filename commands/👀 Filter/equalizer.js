    ...
    try {
      ...
      if (!args.length || (!client.eqs[args[0].toLowerCase()] && args[0].toLowerCase() != `none`))
        return message.channel.send(new MessageEmbed()
          ...
          .setDescription(`Valid Equalizers:\n\`music\`, \`pop\`, \`electronic\`, \`classical\`, \`rock\`, \`full\`, \`gaming\`, \`bassboost\`,  \`earrape\`, \`Owlnest\`\n\nUsage: \`${prefix}equalizer <Type>\`\n\nExample: \`${prefix}equalizer owlnest\``)
        );
      ...
      switch (level) {
        ...
        case `owl`:
        case `owlnest`:
          player.setVolume(player.volume + 150);
          player.setEQ(client.eqs.earrape);
          break;
          case `owln`:
            player.setVolume(player.volume + 1500);
            player.setEQ(client.eqs.earrape);
            break;
      }
      return message.channel.send(new MessageEmbed()
